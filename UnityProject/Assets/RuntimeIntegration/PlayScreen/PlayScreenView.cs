...
namespace Flylingual.PlayScreen
{
    ...
    public sealed class PlayScreenView : MonoBehaviour
    {
        ...
        void Build()
        {
            ...
            var subtitle = Label(GameLanguage.Text("ハエと話し、行動と神経活動を観察する", "Talk to a fly and observe its actions and neural activity"), 14, cream); subtitle.style.marginLeft = 16; subtitle.style.opacity = .82f; header.Add(subtitle);
            header.Add(Spacer()); statusLabel = Label(GameLanguage.Text("接続を確認中", "Checking connection"), 13, mint); statusLabel.style.unityTextAlign = TextAnchor.MiddleRight; header.Add(statusLabel); root.Add(header);
            ...
            var gameTitle = Label(GameLanguage.Text("ゲーム画面 / GAME VIEW", "GAME VIEW"), 13, amber, FontStyle.Bold); gameTitle.style.marginLeft = 14; gameTitle.style.marginTop = 12; gameFrame.Add(gameTitle);
            ...
            var neuralTitle = Label(GameLanguage.Text("脳・神経活動", "Brain activity"), 13, mint, FontStyle.Bold);
            neuralTitle.tooltip = GameLanguage.Text("橙色は実測発火と短い残光。シアン・紫は起動時の基準からの平均膜電位変化で、発火とは別です。膜電位は受信した細胞だけを表示します。灰色は細胞の位置です。", "Orange shows observed spikes with a short afterglow. Cyan and purple show mean membrane potential changes from the startup baseline, separately from spikes. Voltage is shown only for received cells. Gray marks cell positions.");
            neuralCard.Add(neuralTitle);
            var neuralLegend = Row("neural-legend"); neuralLegend.style.flexWrap = Wrap.Wrap;
            neuralLegend.Add(Label(GameLanguage.Text("● 発火  ", "● Spikes  "), 12, new Color(1f, .48f, .12f)));
            neuralLegend.Add(Label(GameLanguage.Text("● 電位上昇  ", "● Voltage rise  "), 12, new Color(.1f, .85f, 1f)));
            neuralLegend.Add(Label(GameLanguage.Text("● 電位低下", "● Voltage fall"), 12, new Color(.76f, .38f, 1f)));
            ...
            portraitCard.Add(Label(GameLanguage.Text("ハエリンガル / FLYLINGUAL", "FLYLINGUAL"), 13, amber, FontStyle.Bold));
            portrait = new FlyPortraitElement(); portrait.style.flexGrow = 1; portrait.style.minHeight = 96; portraitCard.Add(portrait);
            portraitSource = Label(GameLanguage.Text("表現の根拠: 接続状態", "Expression source: connection state"), 12, mint); portraitCard.Add(portraitSource);
            portraitObservation = Label(GameLanguage.Text("気持ちはまだわかりません", "Feelings are not yet known"), 12, cream); portraitObservation.style.whiteSpace = WhiteSpace.Normal; portraitCard.Add(portraitObservation);
            ...
            startButton = MakeButton(GameLanguage.Text("会話のみ開始", "Start chat only"), () => controller?.StartConversation()); actions.Add(startButton);
            stopButton = MakeButton(GameLanguage.Text("会話を終了", "End conversation"), () => controller?.StopConversation()); actions.Add(stopButton);
            var emergency = MakeButton(GameLanguage.Text("緊急停止", "Emergency stop"), () => controller?.EmergencyStop()); emergency.style.backgroundColor = new Color(.64f, .18f, .13f); emergency.style.color = Color.white; emergency.style.unityFontStyleAndWeight = FontStyle.Bold; actions.Add(emergency);
            muteButton = MakeButton(GameLanguage.Text("マイクをミュート", "Mute microphone"), () => { if (controller != null) controller.SetMicrophoneMuted(!controller.MicrophoneMuted); }); actions.Add(muteButton);
            voiceButton = MakeButton(GameLanguage.Text("声で操作を有効にする", "Enable voice controls"), () => controller?.EnableVoiceActions()); actions.Add(voiceButton);
            actions.Add(Spacer()); settingsButton = MakeButton(GameLanguage.Text("設定と診断", "Settings and diagnostics"), ToggleSettingsDrawer); actions.Add(settingsButton);
            controlModeLabel = Label(GameLanguage.Text("操作状況: 停止中", "Controls: stopped"), 13, mint, FontStyle.Bold); controlModeLabel.style.marginTop = 5; controls.Add(controlModeLabel);
            ...
            captionLabel = Label(GameLanguage.Text("字幕: 接続待ち", "Captions: waiting for connection"), 14, cream); captionLabel.style.marginTop = 7; captionLabel.style.whiteSpace = WhiteSpace.Normal; captionLabel.style.maxHeight = 42; captionLabel.style.overflow = Overflow.Hidden; controls.Add(captionLabel);
        ...
        void BuildSettings(VisualElement settings)
        {
            ...
            scroll.Add(Label(GameLanguage.Text("マイク入力と出力", "Microphone and audio"), 12, mint, FontStyle.Bold));
            ...
            applyButton = MakeButton(GameLanguage.Text("停止状態で設定を適用", "Apply settings while stopped"), ApplySettings); scroll.Add(applyButton);
            ...
            sendButton = MakeButton(GameLanguage.Text("指示を送る", "Send command"), SendText); scroll.Add(sendButton);
            scroll.Add(Label(GameLanguage.Text("診断", "Diagnostics"), 12, mint, FontStyle.Bold));
            ...
            scroll.Add(Label(GameLanguage.Text("会話全文", "Conversation transcript"), 12, mint, FontStyle.Bold));
            transcriptLabel = new Label(GameLanguage.Text("会話の接続待ち", "Waiting for conversation")); transcriptLabel.style.whiteSpace = WhiteSpace.Normal; scroll.Add(transcriptLabel);
        }
