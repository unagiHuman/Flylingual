...
| Ruler | 幅16・長さ約25.6 | 橋の方向に合わせて歩く（2026-09-14に幅を2倍へ変更） |
...
## 橋の難易度緩和と方向案内（2026-09-14）

定規橋 `RulerBridge` の幅を8から16へ拡張した。Colliderと描画面を合わせ、縁と目盛りを移動し、接合部の重複面を除去するMeshを再生成した。生成用Builderも同じ幅へ更新した。

既存の約3mの局所観測で前方・斜め前方の橋を捉えると、「前右に橋（定規）。少し右に向きを合わせて。」のように観測済みの方向と向き合わせを日英で案内する。音声版とテキスト版に対応。同じ方向・向きの案内を繰り返さず、非常に近い縁の危険を優先する。プレイヤーの発話優先中は通知済みにせず、その後も橋が見えていれば案内できる。未観測の遠方経路や安全を保証せず、自動操作も追加しない。

関連Pythonテスト46件が成功。Editorで幅16のColliderとMesh、および橋上27点の支持面を確認した（[静的検証](../../artifacts/bridge-easier-20260914/geometry.txt)、[拡幅後の静止描画](../../artifacts/bridge-easier-20260914/bridge-topdown.png)）。実Brainでの通行・音声の実聴は今回未実施。通し試験はユーザーの人手確認に残す。既存の提出用Windows ZIPはこの変更では更新していない。
