...
2026-09-15のユーザー指示により、確定したプレイヤーの移動要求が解釈後もclarifyとなる場合に限り、Unityから届いた新鮮な既知コースのroute hintを用いて、短い期限付きActionを試みる例外を設ける。これはゲーム側の案内補助であり、GPTが未知の地形・経路を推測する許可ではない。hintの欠測・stale時に推測せず、操作権、世代、鮮度、時間上限、取消、安全停止を維持する。質問・雑談は既存の移動を変更せず、ハエ自身の発言から操作を作らない。到達・危険回避を保証しない。既存の刺激→raw神経出力→decoder→motor経路を維持し、route hintからmotorを直接生成しない。経路選択をMaleCNSの視覚・感情・学習・神経反応の実測結果と呼ばない。英語の自然なキャラ表現と検証状況は[英語表示とコース案内](windows/English-Only-Route-Guidance.md)を参照する。これは実装範囲の更新であり、最新の実Brain／実API／Windows Unityの合格を示さない。
...
詳細は[指示の保持・継続](integration/Persistent-Intents.md)。これは受付鮮度・Brain適用待ち・安全監視の無期限化ではない。距離指定の対象、観測、終了理由は[距離指定の移動意図](integration/Distance-Intents.md)を参照する。
...
2026-09-13の応答短縮依頼により、完結した明確な短文は全文一致の規則で同じ意図proposalへ変換し、追加のLLM呼出しを省略する。あいまいな指示、文脈依存、否定・訂正などは既存の意味解釈へ渡す。音声のGPT Live、操作権・世代・鮮度検査、神経出力を経由する身体制御は維持する。delegationとtranscriptの受付を統合し、純粋旋回では前方障害だけを拒否理由から外す。仕様・実測の区別と発話境界の限界は[音声Actionレイテンシ](integration/Voice-Action-Latency.md)を参照する。

2026-09-13の追加合意により、Fast Rule → Local LLM を優先し、確定発話の明示方向を含む local clarify の一部だけ Responses に一度再解釈させる構成を追加する。`localIntentResponsesFallback` を明示有効にした場合だけ使用し、通信障害や数量検証の拒否は再解釈しない。音声返答生成を操作の前提にせず、全体判定期限と最新世代の検査を維持する。実装条件と未検証事項は [Hybrid Intent Routing](windows/Hybrid-Intent-Routing.md) を参照する。
