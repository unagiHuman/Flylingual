...
Backendへ追加。既存Actionからなる限定的な計画であり、完全な自律ナビゲーションではない。
共通設計正本の制御境界を維持する。
同日追加の[指示の保持・継続](Persistent-Intents.md)により、明示的な継続要求は停止・次操作まで保持できる。以下の期間未指定の新規操作は引き続き有限動作とする。
...
前者は現在段階・期限・条件の維持、後者は条件だけの追加であり、新しい計画として開始し直さない。明示的な期限変更・継続指定だけを別途適用する。
...
すべてのkind=planを危険監視し、通常Actionには明示的な条件追加が受理された場合だけ監視を追加する。manual入力を勝手に置換しない。
...
- 有限動作の既存defaultActionMs=4000、maxActionMs=8000を変更しない。端末設定が短ければその値を使う。明示継続のuntil_next_commandは期限nullとし、nudgeは有限に限定する。
- 指示の鮮度はResponses解釈開始からmaxIntentAgeMs（既定8000ms）未満で判定し、旧Planの取消待ち後にも確認する。有限Planの絶対実行期限は受付時からvalidForMsとし、API時間を差し引かない。複数stepで同じ期限を共有し、「そのまま」や条件追加では期限を延ばさない。明示的な期限変更・継続指定は[保持仕様](Persistent-Intents.md)に従う。
