  ...
  class FlyAudio {
    ...
    async start() {
      ...
      try {
        ...
        this.node.port.onmessage=event=>{
          if(!this.active || !this.allowed() || this.playback.size || ctx.currentTime<this.echoUntil) { this.pending=[]; this.carry=new Float32Array(0); this.position=0; return; }
    ...
    capture(input,rate) {
      ...
      while(this.pending.length>=2400) {
        ...
        this.send({type:'audio',audio:btoa(String.fromCharCode(...bytes))});
      }
    }
    stopPlayback() { for(const node of this.playback){node.onended=null;try{node.stop();node.disconnect();}catch{}}this.playback.clear();this.nextTime=0;this.echoUntil=(this.context?.currentTime||0)+.2; }
    stopMic() {
      ...
      this.pending=[];this.carry=new Float32Array(0);this.position=0;this.notify();
    }
    ...
    async play(encoded) {
      ...
      try {
        ...
        const bytes=Uint8Array.from(binary,c=>c.charCodeAt(0)),view=new DataView(bytes.buffer),buffer=ctx.createBuffer(1,bytes.length/2,24000),channel=buffer.getChannelData(0);
        ...
        const start=Math.max(ctx.currentTime,this.nextTime);this.nextTime=start+duration;this.echoUntil=this.nextTime+.2;
        node.onended=()=>{node.disconnect();this.playback.delete(node);};node.start(start);
