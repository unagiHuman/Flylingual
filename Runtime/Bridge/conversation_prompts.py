...
Do not delegate to the backend when: 挨拶、雑談、既に届いた結果の繰返し。移動意図や方向が不明、指示が矛盾、停止条件が未対応なら短く確認します。「話すのをやめて」は発話停止であり、ハエの停止要求とは区別します。
...
Do not delegate to the backend when: Greeting, chatting or repeating an already supplied result. Ask briefly if movement intent or direction is unclear, directions conflict, or a stopping condition is unsupported. "Stop talking" stops speech and is distinct from stopping the fly.
