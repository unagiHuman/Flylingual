...
parser=argparse.ArgumentParser();parser.add_argument('--output',default='artifacts/windows-malecns/ordered-adhesion');options=parser.parse_args()
...
args=[str(ROOT/'artifacts/windows/m1/player/FlyAscent.exe'),'-physicsDiagnostic','-diagnosticOrderedAdhesion','-diagnosticBatch','steering','-diagnosticInitial',str(ROOT.parent/'Flytest/artifacts/windows/physics-diagnostic/controlled-initial.json'),'-fixedFixtures',str(ROOT/'Docs/windows/physics-diagnostic/fixtures'),'-demoOutput',str(out),'-screen-fullscreen','0','-screen-width','800','-screen-height','600','-logFile',str(out/'Player.log')]
